...
describe('scoreboard', () => {
  it('lists the highest score first', () => {
    const entries = scoreboard(students, {
  ...
  it('breaks ties alphabetically so the order is stable', () => {
    expect(scoreboard(students, EMPTY_LEDGER).map((entry) => entry.name)).toEqual([
      'Ana',
      'Ben',
      'Cara',
    ]);
