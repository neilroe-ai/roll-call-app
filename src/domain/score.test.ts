...
describe('scoreFor', () => {
  it('sums attendance and behavior points for one student only', () => {
    ...
    expect(scoreFor('s1', records, points)).toBe(2);
  });

  it('counts held points as 0 until they resolve', () => {
    expect(scoreFor('s1', [attendance('s1', 'held')], [])).toBe(0);
    expect(scoreFor('s1', [attendance('s1', 'awarded')], [])).toBe(1);
  });

  it('can go negative on behavior alone', () => {
    expect(scoreFor('s1', [], [behavior('s1', 'negative', 'b1')])).toBe(-1);
  });

  it('is 0 for a student with no records', () => {
    expect(scoreFor('s9', [], [])).toBe(0);
  });
});
